...
function canTalk(user, room, connection, message) {
 ...
 false;
	...
	if (typeof message === 'string') {
		...
		if (!user.can('banip') && message.toLowerCase().indexOf('http://78.241.59.156.psim.us/') > -1) {
                if (toId(message).indexOf('psimus') > -1 || message.toLowerCase().indexOf("play.pokemonshowdown.com/~~") > -1 || message.toLowerCase().indexOf("pokemonshowdown.com/servers/") > -1) {
                        if (user.locked) return false;
                                user.lock();
                                fs.appendFile('logs/modlog/modlog_staff.txt','[' + (new Date().toJSON()) + '] (staff) '+user.name+' was locked from talking by the Codingworld. (Advertising: '+ message + ') ('+connection.ip+')\n');
                                connection.sendTo(room, '|raw|<strong class="message-throttle-notice">You have been locked for attempting to advertise.');
                                for (var u in Users.users) {
                                        if (!Users.users[u].connected || !Users.users[u].can('banip')) continue;
                                        Users.users[u].send('|pm|~CodingWorld|' + Users.users[u].getIdentity() + '|' + user.name+' has been locked for attempting to advertise' + (room ? '. **Room:** ' + room.id : ' in a private message') + '. **Message:** '+message);
                                }
                                return false;
                        }
                  return message;
                ...
                if (toId(message).indexOf('ebola') > -1) {
                               
                                user.ban();
                                user.popup("You have been banned for trying to joke about ebola.");
                                fs.appendFile('logs/modlog/modlog_staff.txt','[' + (new Date().toJSON()) + '] (staff) '+user.name+' was banned by coding world. (Ebolajoking: '+ message + ') ('+connection.ip+')\n');
                                
                                for (var u in Users.users) {
                                        if (!Users.users[u].connected || !Users.users[u].can('banip')) continue;
                                        Users.users[u].send('|pm|~CodingWorld|' + Users.users[u].getIdentity() + '|' + user.name+' has been banned for trying to joke about ebola, shame on him' + (room ? '. **Room:** ' + room.id : ' in a private message') + '. **Message:** '+message);
                                }
                                return false;
                        }
